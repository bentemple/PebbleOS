...
// nothing, so no shred reason can reach a third-party app. The other half of
// that rule lives in the kernel, which does not publish the event at all for a
// duress wipe; see test_security_lock_shred.
...
void test_security_shred_service__any_other_task_is_a_bug(void) {
  s_current_task = PebbleTask_KernelMain;

  cl_assert_passert(security_shred_service_subscribe(prv_handler));
  cl_assert_passert(security_shred_service_unsubscribe());

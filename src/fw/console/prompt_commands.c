...
void command_security_ui(void) {
  ...
  if (!info.interlock) {
    prompt_send_response("Couldn't allocate semaphore for security ui");
    return;
  }
...
void command_security_unlock(const char *digits) {
  ...
  if (!info.interlock) {
    prompt_send_response("Couldn't allocate semaphore for security unlock");
    return;
  }
...
void command_security_session(const char *is_open_str) {
  ...
  if (!info.interlock) {
    prompt_send_response("Couldn't allocate semaphore for security session");
    return;
  }

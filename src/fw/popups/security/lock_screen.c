...
//! @param refused true if the service turned the entry away without comparing
//!        it, because the backoff had not run out.
static void prv_show_failure(uint8_t attempts_remaining, bool refused) {
  char message[SECURITY_PIN_MESSAGE_BUF_SIZE];

  if (refused) {
    // Nothing was compared, so nothing new has happened: say how long is left
    // and queue no wipe. The wipe belongs to the attempt that spent the budget,
    // which is the one below.
    const uint32_t remaining_s = security_lock_get_lockout_remaining_s();
    const uint32_t minutes = (remaining_s + 59) / 60;
    char format[SECURITY_PIN_MESSAGE_BUF_SIZE];
    i18n_get_with_buffer((minutes == 1) ? i18n_noop("Locked out, try in 1 min")
                                        : i18n_noop("Locked out, try in %u min"),
                         format, sizeof(format));
    sniprintf(message, sizeof(message), format, (unsigned)minutes);
  } else if (security_lock_attempts_exhausted()) {
...
static void prv_submit(const char *digits, uint8_t len, void *context) {
  // Asked before the entry, because verifying is what moves it: a backoff that
  // was already running is what makes this entry a refusal rather than a guess.
  const bool refused = (security_lock_get_lockout_remaining_s() > 0);
  ...
  prv_show_failure(attempts_remaining, refused);
}

//! BACK hides the pad, once it has no digits left to take back. It does not
//! unlock anything: the state stays Locked, the UI lockout stays held, and the
//! next button press raises the pad again from launcher_handle_button_event().
//!

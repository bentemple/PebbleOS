...
console connection, because the daemon owns the console port exclusively::

    pbl qemu &
    pbl harness serve &

    python3 tools/security_lock_stress.py --iterations 100
    python3 tools/security_lock_stress.py --iterations 20 --prefill 80
    python3 tools/security_lock_stress.py --self-test

The harness socket defaults to `harness.sock` in the build directory `pbl`
would use; `--socket` names another one.
...
trigger can queue behind an in-flight ``notif test``), when the wipe's own
``Shredding:`` line arrived relative to the traffic starting -- ``--`` if it
never did, which means the iteration collided with nothing -- how many
notifications were delivered, and the verdict. The closing summary reports iterations
...
#: The checkout, and the build in it, found the way `pbl` finds them.
WORKSPACE = Workspace.find(os.path.dirname(os.path.abspath(__file__)))
DEFAULT_SOCK = BuildDir(WORKSPACE.build_dir, WORKSPACE.topdir).join("harness.sock")
...
#: What a driver bug in this file looks like. Enumerated rather than bare: a
#: stress tool must not die on a traceback halfway through a long run, but it
#: must not swallow a genuine defect in itself either.
ITERATION_FAILURES = (
    ArithmeticError,
    AssertionError,
    AttributeError,
...
#: `security status`'s state field: 0 off, 1 armed (on, unlocked), 2 locked.
STATE_ARMED, STATE_LOCKED = 1, 2

#: The PIN every iteration leaves configured. Which value it is does not
#: matter; that one is *set* does -- see reset_state().
STRESS_PIN = "1234"
...
#: `prv_shred()`'s own first line (`shred.c`). The console prompt returns
#: before this, because the trigger only queues a launcher-task callback, so
#: this is the only thing that says the collision window actually opened.
SHRED_START_RE = r"Shredding:"
...
def _call(sock_path, cmd, args=None, timeout=120.0):
    ...
    Deliberately its own copy rather than an import of `pbl harness`'s client:
    the extension command is loaded by the CLI from a path in `pbl.yml`, and
    the wire format is two lines of JSON, so speaking it directly costs less
    than reaching into the command that serves it.
    """
...
class Watch:
    ...
    def wait(self, pattern, since=None, timeout=30.0):
        """Block until a log line after `since` matches, in the daemon's ring."""
        return _call(
            self.sock,
            "wait",
            {"pattern": pattern, "since": since, "timeout": timeout},
            timeout=timeout + 30.0,
        )
...
class Traffic:
    ...
    def join(self, timeout=30.0):
        """Let the configured window elapse, then stop.

        Setting `_stop` first would end the traffic at whatever instant the
        caller reached here. That is the wrong instant: `security shred`
        marshals to the launcher task and the console prompt returns *before*
        the wipe, so cutting the traffic on the prompt's return very likely
        removes it before the collision window opens -- and a clean run then
        proves nothing at all. The thread stops itself when its window ends;
        `_stop` is only the backstop for a thread that will not.
        """
...
class Stress:
    ...
    def reset_state(self):
        """Get back to armed-but-unlocked, with a PIN this tool knows.

        Both halves are load-bearing, and each was wrong once:

        * **unlocked** -- `notifications_add_notification()` drops the
          notification outright on a locked watch, so a locked one generates no
          `notifstr` traffic at all and the window cannot be hit;
        * **armed** -- `security shred` refuses on a watch whose lock is
          switched off, logging "Security lock is off; not shredding" and
          returning without ever reaching `prv_shred()`. Clearing the PIN is
          what switches the lock off, so "reset by clearing the PIN" is not a
          neutral starting state: it is the one state in which the trigger
          does nothing at all.
        """
        status = self.watch.security_status()
        if status is None:
            return False
        if status.get("state") == STATE_LOCKED:
            # The PIN may be unknown (set by an earlier run), so clear rather
            # than guess; the one below is set again straight after.
            self.watch.prompt("security clear pin", timeout=10.0)
            status = self.watch.security_status() or {}
        if status.get("pin_len") != len(STRESS_PIN):
            self.watch.prompt(f"security set pin {STRESS_PIN}", timeout=10.0)
        self.watch.prompt("security deadlines 0 0", timeout=10.0)
        status = self.watch.security_status()
        return bool(status) and status.get("state") == STATE_ARMED

    def arm_trigger(self, trigger):
        """Prepare whatever the trigger needs, and return the console command.

        `lock` needs a PIN before `security lock` will do anything, which
        reset_state() has already seen to. It is kept in the mix because it is
        a different entry point, but note that it locks the watch *first*,
        which closes the notification window -- so it is a control, not the
        main event. That asymmetry is reported.
        """
        if trigger == "lock":
    ...
    def iteration(self, index):
        ...
        if not self.reset_state():
            return {
                ...
                "failed": True,
                "reasons": [
                    "could not reach a known armed-but-unlocked state before the run"
                ],
                "logs": [],
            }
        ...
        # The prompt returning does not mean the wipe ran: `command_security_shred`
        # marshals to the launcher task and answers straight away. `Shredding:`
        # (shred.c) is the wipe's own first line, so it is what says the window
        # was actually opened -- and it is the only thing that distinguishes an
        # iteration that collided from one that fired into an idle watch.
        wipe = self.watch.wait(
            SHRED_START_RE, since=since_seq, timeout=options.wipe_wait_s
        )
        wipe_seen = bool(wipe.get("matched"))
        wipe_ms = (time.monotonic() - (t0 + traffic_delay)) * 1000.0 if wipe_seen else None
        ...
        return {
            ...
            "wipe_seen": wipe_seen,
            "wipe_offset_ms": None if wipe_ms is None else round(wipe_ms, 1),
    ...
    def run(self):
        ...
        for index in range(1, options.iterations + 1):
            ...
            except ITERATION_FAILURES as exc:
                result = {
                    "index": index,
                    "trigger": "?",
                    "failed": True,
    ...
    def report_iteration(self, result):
        ...
        wipe = result.get("wipe_offset_ms")
        print(
            ...
            f"wipe={'--' if wipe is None else f'{wipe:+.0f}ms'} "
    ...
    def summary(self):
        ...
        print(
            f"wipes observed        {self.wipes}  "
            "('Shredding:' seen after the trigger)"
        )
        ...
        if not self.wipes:
            print(
                "\nNO WIPE WAS EVER OBSERVED: the trigger never reached "
                "prv_shred(), so nothing collided with anything and this run "
                "is not evidence about the race at all."
            )
        if not hunted:
            print(
...
def build_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=10.0,
        help="how long to wait for the wipe's own 'Shredding:' line after the "
        "trigger; the console prompt returns well before it",
    )
